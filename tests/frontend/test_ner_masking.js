...
{
    const text = 'Wie hat sich Marie gefühlt, als ich mit Anna in München war?';
    const entities = [
        { entity: 'B-PER', score: 0.9996, index: 4, word: 'Marie', start: null, end: null, entity_group: 'PER' },
        { entity: 'B-PER', score: 0.9993, index: 16, word: 'Anna', start: null, end: null, entity_group: 'PER' },
        { entity: 'B-LOC', score: 0.9997, index: 18, word: 'München', start: null, end: null, entity_group: 'LOC' },
    ];
    const tokenMap = { 'Marie': '[PER_1]', 'Anna': '[PER_2]', 'München': '[LOC_1]' };

    const { masked, assigned } = applyMaskingLogic(text, entities, tokenMap);

    assert('Text ist nicht leer', masked.length > 0);
    assert('Text enthält keinen Klarnamen "Marie"', !masked.includes('Marie'));
    assert('Text enthält keinen Klarnamen "Anna"', !masked.includes('Anna'));
...
console.log('\n2. Nachweis dass alter Bug (!== undefined) falsch war:');
{
    ...
    const entity = { word: 'Marie', start: null, end: null, entity_group: 'PER' };
    const tokenMap = { 'Marie': '[PER_1]' };
    ...
    // → Ergebnis: "" + "[PER_1]" + "Hallo Marie" = "[PER_1]Hallo Marie" (Dopplung!)
...
{
    const text = 'Marie lebt in Berlin';
    const entities = [
        { word: 'Marie', start: 0, end: 5, entity_group: 'PER' },
        { word: 'Berlin', start: 14, end: 20, entity_group: 'LOC' },
    ];
    const tokenMap = { 'Marie': '[PER_1]', 'Berlin': '[LOC_1]' };

    const { masked } = applyMaskingLogic(text, entities, tokenMap);

    assert('Text enthält kein "Marie"', !masked.includes('Marie'));
...
{
    const text = 'Anna ist da';
    const entities = [
        { word: 'Anna', start: 0, end: 4, entity_group: 'PER' },
    ];
    const tokenMap = { 'Anna': '[PER_1]' };

    const { masked } = applyMaskingLogic(text, entities, tokenMap);

    assert('Text enthält kein "Anna"', !masked.includes('Anna'));
