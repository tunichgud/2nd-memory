...
3. **"Was kann ich Marie zum Geburtstag schenken?"**
   - ❌ Problem: Maries Interessen müssen aus Nachrichten, Fotos, Reviews extrahiert werden
   - ❌ Problem: Braucht Multi-Step-Reasoning (Schritt 1: Finde Infos über Marie → Schritt 2: Leite Interessen ab)
...
- **Kein Multi-Hop Reasoning**: Anfrage "Was kann ich Marie schenken?" braucht:
  1. Finde Nachrichten von Marie
  2. Extrahiere Interessen aus Messages
  3. Suche Fotos von Marie bei Aktivitäten
...
def extract_facts(context: str, question: str) -> str:
    ...
    Beispiel:
        context = "Marie: Ich liebe Yoga und Klettern!"
        question = "Was sind Maries Hobbies?"
        → "Yoga, Klettern"
    """
    messages = [
...
### 4.3 "Was kann ich Marie zum Geburtstag schenken?"

**Alt:**
```
retrieve_v2("Marie Geburtstag schenken", persons=["Marie"])
→ Findet random Messages/Fotos mit Marie
→ LLM halluziniert Geschenkideen ohne echte Datengrundlage
```

**Neu:**
```
analyze_query("Was kann ich Marie zum Geburtstag schenken?")
...
→ Execute Sub-Queries:
   [Schritt 1] search_messages(personen=["Marie"])
   [Schritt 2] search_photos(personen=["Marie"])
   [Schritt 3] extract_facts(context, "Was sind Maries Hobbies?")
              → "Yoga, Klettern, Bücher"
   [Schritt 4] LLM: "Basierend auf Maries Interessen (Yoga, Klettern):
                - Yoga-Matte
                - Klettergurt
                - Buch: ..."
```
...
[Schritt 2: Finde Nachrichten mit Marie]
...
BISHERIGE ERKENNTNISSE:
- User war mit Marie in München
- Marie erwähnt Outdoor-Aktivitäten
```
