...
| **Ziel (neu)** | `wa_{chat_id}_{timestamp}_{sender_hash}` | `wa_491786838260@c.us_1556282520_josh` | ✅ Deterministisch |
...
```
WhatsApp Chat 'WhatsApp-Chat mit Sarah Ohnesorge.txt':
[26.04.19 14:42] Josh: Hi
[27.04.19 09:01] Sarah Ohnesorge: Es war wirklich süß
```

**Extrahierbare Informationen:**
- ✅ **Datum/Zeit**: `26.04.19 14:42` → Unix timestamp `1556282520`
- ✅ **Sender**: `Josh` → Hash: `josh`
- ✅ **Chat-Kontakt**: `Sarah Ohnesorge` (aus Dateiname)
- ✅ **Chat-ID**: Bereits in Metadata: `chat_id: "491786838260@c.us"`
...
client.on('message_create', async msg => {
    ...
    msg.from           // "491786838260@c.us"
    msg.author         // "491798924094@c.us" (sender in groups)
});
```
...
1. **chat_id** (z.B. `491786838260@c.us`) eindeutig pro Chat
...
# Beispiel TXT-Import
id = "wa_491786838260@c.us_1556282520_josh"
...
def normalize_sender(sender: str) -> str:
    ...
    'Sarah Ohnesorge' → 'sarah_ohnesorge'
    '491786838260@c.us' → '491786838260'
    'Ich' → 'me'
    """
    sender = sender.lower()
...
def parse_txt_line_to_id(line: str, chat_id: str) -> Optional[str]:
    ...
    Input:  "[26.04.19 14:42] Josh: Hi there"
    Output: "wa_491786838260@c.us_1556282520_josh"
    """
    import re
...
def migrate_txt_imports():
    """Migriert TXT-Import Messages (3025 Sarah Messages)."""

    col = get_collection("messages")

    # Hole alle TXT-Import Messages
    old_msgs = col.get(
        where={"chat_name": "Sarah Ohnesorge"},
    ...
    for i, old_id in enumerate(old_msgs['ids']):
        ...
        # Generiere neue ID
        chat_id = meta.get('chat_id', '491786838260@c.us')
...
def test_deduplication():
    ...
    # TXT-Import
    txt_line = "[26.04.19 14:42] Josh: Hi there"
    ...
    assert txt_id.startswith("wa_491786838260@c.us_1556282520_josh")
    assert live_id_no_collision == "wa_491786838260@c.us_1556282520_josh"
```
...
4. ✅ **3025 Sarah Messages** erfolgreich migriert
...
**Problem:**
```
[26.04.19 14:42] Josh: Dies ist eine
...
**Problem:**
```
[26.04.19 14:42] Sarah: <Medien ausgeschlossen>
```
...
**Problem**: Josh sendet 2 Messages in Sekunde 1556282520
