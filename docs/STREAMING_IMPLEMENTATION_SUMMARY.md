...
Wir haben das RAG-System von 2nd Memory mit **Real-Time Denkprozess-Streaming** erweitert.
...
```
User Query: "Wo war ich gestern mit Anna?"
    ↓
┌─────────────────────────────────────────────────────┐
...
4. **Answer streamt live** (ab ~1.4s)
   ```
   Gestern warst du mit Anna im Restaurant...
...
3. **Test-Queries:**
   ...
   **Test 2: Complex Query (mit Thoughts)**
   ```
   User: "Wo war ich letztes Jahr im August mit Anna?"

   Erwartete Events:
   - query_analysis (Personen: Anna, Datum: 2025-08)
