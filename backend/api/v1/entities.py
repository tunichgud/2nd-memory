...
class ChatMetadata(BaseModel):
    chat_name: str = Field(..., description="Name der Person im Chat (z.B. 'Marie')")
...
class LinkEntityRequest(BaseModel):
    entity_name: str = Field(..., description="Der Name, der zugewiesen werden soll (z.B. 'Marie')")
...
async def unlink_face(req: UnlinkFaceRequest):
    ...
    # 2. Sync Photo Persons (damit das Foto nicht mehr Marie taggt, wenn das Gesicht weg ist)
