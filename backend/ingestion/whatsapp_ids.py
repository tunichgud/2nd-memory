...
def normalize_sender(sender: str) -> str:
    ...
    Args:
        sender: Sender-Name (z.B. "Sarah Ohnesorge", "491786838260@c.us", "Ich")

    Returns:
        Normalisierter Sender (z.B. "sarah_ohnesorge", "491786838260", "me")

    Examples:
        >>> normalize_sender("Sarah Ohnesorge")
        'sarah_ohnesorge'
        >>> normalize_sender("Ich")
        'me'
        >>> normalize_sender("491786838260@c.us")
        '491786838260'
    """
    if not sender:
        return 'unknown'
...
def generate_message_id(
    ...
    Examples:
        >>> generate_message_id("491786838260@c.us", 1556282520, "Josh")
        'wa_491786838260@c.us_1556282520_josh'
        >>> generate_message_id("123@c.us", 1000, "Josh", "Hello")
...
def parse_txt_line_to_id(line: str, chat_id: str) -> Optional[str]:
    ...
    Args:
        line: Zeile aus WhatsApp TXT-Export (z.B. "[26.04.19 14:42] Josh: Hi there")
        chat_id: WhatsApp Chat-ID (z.B. "491786838260@c.us")
    ...
    Examples:
        >>> parse_txt_line_to_id("[26.04.19 14:42] Josh: Hi", "491786838260@c.us")
        'wa_491786838260@c.us_1556282520_josh'
...
def parse_txt_line(line: str) -> Optional[Dict[str, Any]]:
    ...
    Examples:
        >>> parse_txt_line("[26.04.19 14:42] Josh: Hi there")
        {'timestamp': 1556282520, 'sender': 'Josh', 'message': 'Hi there'}
    """
    pattern = r'\[([^\]]+)\]\s*([^:]+):\s*(.+)'
    match = re.match(pattern, line)
...
def _run_self_tests():
    ...
    msg_id = generate_message_id("491786838260@c.us", 1556282520, "Josh")
    ...
    parsed = parse_txt_line("[26.04.19 14:42] Josh: Hi there")
    assert parsed is not None
    assert parsed['timestamp'] == 1556282520
    assert parsed['sender'] == 'Josh'
