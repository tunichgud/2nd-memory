...
2nd-memory nutzt jetzt **Google OAuth 2.0** für sichere Authentifizierung. Das System ist als **Single-User-Installation** konzipiert: Der erste Google-Account, der sich anmeldet, wird zum Owner – weitere Accounts werden abgewiesen.
...
2. Erstelle ein neues Projekt (z.B. "2nd-memory-auth")
...
6. Backend erstellt Session (30 Tage Expiry)
   ↓
7. Backend setzt HttpOnly-Cookie: 2nd_memory_session=...
   ↓
...
Jeder API-Request:
  ↓
1. Middleware liest Cookie: 2nd_memory_session
   ↓
...
2. Lösche DB und starte neu: `rm data/2nd-memory.db && ./start.sh`
...
2. Sollte `2nd_memory_session` sichtbar sein
