...
```bash
# 1. Repository clonen
git clone https://github.com/tunichgud/2nd-memory.git
cd 2nd-memory
...
**Fertig!** 🎉 2nd-memory läuft jetzt lokal.
...
### WhatsApp Bridge (`2nd-memory-whatsapp`)
...
```bash
# SQLite Shell
docker-compose exec backend python -m sqlite3 /app/data/2nd-memory.db

# Database Backup
docker-compose exec backend tar -czf /app/data/backup.tar.gz /app/data/2nd-memory.db
```
...
```bash
CORS_ORIGINS=https://2nd-memory.yourdomain.com
```

### 3. Network Isolation

Alle Services laufen im `2nd-memory` Docker Network:
- Nur exposedte Ports sind öffentlich erreichbar
- Inter-Service Communication über interne DNS-Namen

### 4. Non-Root User

Alle Container laufen als Non-Root User (`2nd-memory:1000`):
```dockerfile
RUN useradd -m -u 1000 2nd-memory
USER 2nd-memory
```
...
```nginx
# /etc/nginx/sites-available/2nd-memory
server {
    listen 80;
    server_name 2nd-memory.yourdomain.com;
...
```bash
sudo ln -s /etc/nginx/sites-available/2nd-memory /etc/nginx/sites-enabled/
sudo nginx -t
...
sudo certbot --nginx -d 2nd-memory.yourdomain.com
```

### 3. Systemd Service (Auto-Start)

```bash
# /etc/systemd/system/2nd-memory.service
[Unit]
Description=2nd Memory Docker Compose
...
Type=oneshot
RemainAfterExit=yes
WorkingDirectory=/opt/2nd-memory
...
sudo systemctl enable 2nd-memory
sudo systemctl start 2nd-memory
```
...
tar -czf /backup/2nd-memory_$DATE.tar.gz \
  data/ \
  .wwebjs_auth/ \
  config.yaml \
  .env
...
**Issues**: https://github.com/tunichgud/2nd-memory/issues
**Docs**: https://docs.2nd-memory.com (TBD)
**Discord**: https://discord.gg/2nd-memory (TBD)
